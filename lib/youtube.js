...
/**
 * Batch-fetch full metadata (snippet + contentDetails) for many video IDs.
 * Each YouTube API call accepts up to 50 IDs. Returns array in same shape
 * as fetchVideoById for each found video.
 */
export async function fetchVideoBatch(videoIds) {
  const key = requireKey();
  const out = [];
  for (let i = 0; i < videoIds.length; i += 50) {
    const chunk = videoIds.slice(i, i + 50);
    const url = `${YT_API}/videos?part=snippet,contentDetails&id=${chunk.join(',')}&key=${key}`;
    const res = await fetch(url);
    const data = await res.json();
    if (data.error) throw new Error(`YouTube API error: ${data.error.message}`);
