...
/**
 * Pick the top 3 videos that best fit a prospect, given prospect notes
 * and a (pre-filtered) library. Library items: [{ id, title, summary,
 * tags, key_points, url, ... }]. Returns each pick with a `reason` field.
 *
 * Optional `refinement` is course-correction context from the rep when
...
export async function findTopMatches(notes, library, refinement = '') {
  ...
  // Slim payload to keep tokens down — large libraries with rich
  // transcripts otherwise hit Anthropic's per-minute input-token caps on
  ...
  const compactLibrary = library.map((v) => ({
    ...
    key_points: (v.key_points || []).slice(0, 6).map((k) => k.slice(0, 140)),
  }));
